...
for game in games:
	...
	until = ''
	date_until = game.find('div', {'style' : 'font-size: x-small;'})
	if (len(date_until.text.split(':')) > 1):
		if (len((date_until.text.split(':')[1]).split(' ')[1]) > 1):
			until = datetime.strptime((date_until.text.split(':')[1]).split(' ')[1], "%m/%d/%Y").strftime("%d.%m")
	...
	if ('RUB' in local_price[3].text):
		...
		# elif (price > 9999 && < 99999):
			# price = price + 99999
		print(link['title'] + ' - ' + str(price) + ' руб. ' + flags[1]['alt'] + ' - ' + until)
